...
const SideBar = () => {
  const { selectedUsers, getUsers, users, setSelectedUsers, unseenMessages } =
    ...
    >
      ...
      <div className="flex flex-col gap-2">
        {filteredUsers.map((data, i) => (
          <div
            key={i}
            onClick={() => setSelectedUsers(data)}
            className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${
              selectedUsers?._id === data._id && 'bg-[#282142]/50'
            }`}
          >
            <img
